...
class Douban:
  ...
  def get_current_movies(self):
    ...
    for card in cards:
      ...
      #item['name'] = card.get_attribute('title')
      time.sleep(4)
      try:
        item['rate'] = card.find_element_by_css_selector('.subject-rate').text
      #item['rate'] = card.get_attribute('data-score')
      except:
        item['rate']=0
  ...
  def get_hot_books(self):
    ...
    wrap_dielement_by_csv = self.dr.find_element_by_css_selector('.section.popular-books')
    ...
    for card in cards:
      ...
      item['rate'] = card.find_s_selector('.average-rating').text
...
class DoubanReporter:
  ...
  def write_movie(self, movie_items):
    ...
    for movie in movie_items:
      movie_item_html = '<li>%s<span style="float:right" class="label label-primary">%s</span></li>' %(movie['name'], movie['rate'])
